...
| `entity.connector.ingest.requested` | Dispatcher BEFORE `connector.ingest(...)`              | `{ kind, connectorId, layerId, contentType, byteLength }`           |
| `entity.connector.ingest.completed` | Dispatcher AFTER `connector.ingest(...)`               | `{ kind, connectorId, layerId, created, updated, warningCount }`    |
...
- Ingest events carry `contentType` + `byteLength` (request) and
  numeric counters (completed) — NEVER raw `bytes` or `filename`.
  Per-row outcomes surface via the regular `entity.<kind>.{created,
updated}` events emitted by the generic store inside the ingest
  loop. ADR [`0014`](../decisions/0014-connector-ingest.md) records
  the second connector method and the synchronous HTTP path.
