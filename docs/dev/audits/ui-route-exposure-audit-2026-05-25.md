...
| DELETE | `/layers/:slug/members/:memberId` | `routes/layers.ts` | `removeLayerMember` defined in `lib/api.ts`, no caller. MembersTab in `LayerSettingsPage` lets users *add* members but offers no remove control. |
