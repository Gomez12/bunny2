/**
 * Phase 4c.4 — pure-logic tests for the Calendar dashboard widget.
 *
 * Mirrors `companies-widget.test.ts` / `contacts-widget.test.ts`: the
 * repo has no DOM runtime yet (see
 * `docs/dev/follow-ups/web-component-tests.md`), so this file covers
 * the matrix the component renders by testing the pure state reducer
 * plus the widget registry contract. The rendered DOM matrix lands
 * once the follow-up wires a DOM runtime.
 *
 * Covered:
 *   - `calendarWidgetView` maps loading / error / ready / empty-by-total
 *     inputs to the exact render branch the component reads.
 *   - The widget registry registers the Calendar widget on import and
 *     exposes a deterministic ordering for `LayerDashboardPage`.
 *   - The widget exposes the literal registration shape promised by the
 *     plan in §4c.4 of `docs/dev/plans/done/phase-04-first-entities.md` —
 *     a rename of the title key or registry kind is caught here before
 *     the dashboard drifts.
 */
import { beforeEach, describe, expect, it } from 'bun:test';
import {
  calendarWidgetView,
  type CalendarWidgetInput,
} from '../src/dashboard/calendar-widget-state';
import {
  listDashboardWidgets,
  registerWidget,
  __resetDashboardWidgetsForTests,
} from '../src/dashboard/widget-registry';

function readyInput(
  overrides: Partial<{
    total: number;
    upcomingNext7d: number;
    withAttendeesLinked: number;
    recentlyEnriched: number;
  }> = {},
): CalendarWidgetInput {
  return {
    status: 'ready',
    stats: {
      total: 4,
      upcomingNext7d: 2,
      withAttendeesLinked: 1,
      recentlyEnriched: 1,
      ...overrides,
    },
  };
}

describe('calendarWidgetView', () => {
  it('returns the loading branch for a loading input', () => {
    expect(calendarWidgetView({ status: 'loading' })).toEqual({ kind: 'loading' });
  });

  it('returns the error branch with the i18n key preserved', () => {
    expect(calendarWidgetView({ status: 'error', errorKey: 'errors.network' })).toEqual({
      kind: 'error',
      errorKey: 'errors.network',
    });
  });

  it('returns the empty branch when total is zero', () => {
    expect(
      calendarWidgetView(
        readyInput({
          total: 0,
          upcomingNext7d: 0,
          withAttendeesLinked: 0,
          recentlyEnriched: 0,
        }),
      ),
    ).toEqual({
      kind: 'empty',
    });
  });

  it('returns the ready branch with the stats payload when total > 0', () => {
    const out = calendarWidgetView(readyInput({ total: 9, upcomingNext7d: 5 }));
    expect(out.kind).toBe('ready');
    if (out.kind === 'ready') {
      expect(out.stats.total).toBe(9);
      expect(out.stats.upcomingNext7d).toBe(5);
    }
  });

  it('treats a single-row ready input as ready (not empty)', () => {
    // Guard against an off-by-one where `total === 1` routes to the
    // empty CTA. The empty branch MUST only fire on 0.
    expect(
      calendarWidgetView(
        readyInput({
          total: 1,
          upcomingNext7d: 0,
          withAttendeesLinked: 0,
          recentlyEnriched: 0,
        }),
      ).kind,
    ).toBe('ready');
  });
});

describe('calendar widget registry', () => {
  beforeEach(() => {
    __resetDashboardWidgetsForTests();
  });

  it('accepts the Calendar widget shape used by the production registration', () => {
    // Mirrors the literal `registerWidget({...})` call inside
    // `apps/web/src/dashboard/CalendarWidget.tsx`. If the registry
    // ever rejects this shape (e.g. a new required field is added
    // without a default), this test catches it before the dashboard
    // renders an empty grid in production.
    const noop = (): null => null;
    registerWidget({
      kind: 'calendar',
      titleKey: 'layer.dashboard.widgets.calendar.title',
      renderer: noop,
      order: 300,
    });
    const widgets = listDashboardWidgets();
    const calendar = widgets.find((w) => w.kind === 'calendar');
    expect(calendar).toBeDefined();
    expect(calendar?.titleKey).toBe('layer.dashboard.widgets.calendar.title');
    expect(calendar?.order).toBe(300);
    expect(typeof calendar?.renderer).toBe('function');
  });

  it('renders after the Companies and Contacts widgets when all three are registered', () => {
    // `order` sort wins: Companies is 100, Contacts is 200, Calendar
    // is 300 — Calendar must come third. Cheap regression guard
    // against accidentally picking a Calendar `order` lower than 200.
    const noop = (): null => null;
    registerWidget({
      kind: 'calendar',
      titleKey: 'layer.dashboard.widgets.calendar.title',
      renderer: noop,
      order: 300,
    });
    registerWidget({
      kind: 'contacts',
      titleKey: 'layer.dashboard.widgets.contacts.title',
      renderer: noop,
      order: 200,
    });
    registerWidget({
      kind: 'companies',
      titleKey: 'layer.dashboard.widgets.companies.title',
      renderer: noop,
      order: 100,
    });
    const order = listDashboardWidgets().map((w) => w.kind);
    expect(order).toEqual(['companies', 'contacts', 'calendar']);
  });
});
